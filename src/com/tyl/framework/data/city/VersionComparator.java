...
import com.tyl.framework.exception.SQLiteAssetException;
...
class VersionComparator implements Comparator<String> {
    ...
    public int compare(String file0, String file1) {
        ...
        if (!m0.matches()) {
            L.w(TAG, "could not parse upgrade script file: " + file0);
            throw new SQLiteAssetException("Invalid upgrade script file");
        }

        if (!m1.matches()) {
            L.w(TAG, "could not parse upgrade script file: " + file1);
            throw new SQLiteAssetException("Invalid upgrade script file");
        }
