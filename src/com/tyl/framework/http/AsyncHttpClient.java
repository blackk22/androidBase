 ...
 * @Package com.firstte
 * @Description AsyncHttpClient是异步HttpClient
 * @version V1.0
 */
public class AsyncHttpClient {
	...
	/** 超时时间，默认10秒 */
	private static final int DEFAULT_SOCKET_TIMEOUT = 100 * 1000;
	/** 默认错误尝试次数 */
	private static final int DEFAULT_MAX_RETRIES = 5;
	/** 默认的套接字缓冲区大小 */
	private static final int DEFAULT_SOCKET_BUFFER_SIZE = 1024*100;
	...
	public AsyncHttpClient() {
		...
		HttpProtocolParams.setUserAgent(httpParams, String.format("firstte/%s (http://www.firstte.cn)", VERSION));
	...
	public void post(Context context, String url, HttpEntity entity,
		...
		HttpUriRequest httpUriRequest=addEntityToRequestBase(new HttpPost(url), entity);
		sendRequest(httpClient, httpContext,httpUriRequest, contentType,
				responseHandler, context);
	}
	 ...
	 * @param url 请求地址
	 * @param content  字符串内容
	 * @param responseHandler  响应接口
	 */
	public void sendBody(String url,String content, AsyncHttpResponseHandler responseHandler){
		...
		sendRequest(httpClient, httpContext, post, null, responseHandler, null);
	 ...
	 * @param responseHandler  响应接口
	 */
	public void sendBody(Context context,String url,String content, AsyncHttpResponseHandler responseHandler){
		HttpPost post=new HttpPost(url);
		post.addHeader("Content-type", "application/x-www-form-urlencoded");
		HttpEntity httpEntity=null;
		try {
		...
		sendRequest(httpClient, httpContext, post, null, responseHandler, context);
	 ...
	 *  一次上传一个文件
	 * @param context
	 * @param filename form表头名
	 * @param files 单个文件
	 * @param responseHandler 异步响应
	 */
	public  void upload(Context context,String url,String filename, File file, RequestParams params,AsyncHttpResponseHandler responseHandler){
		...
		if (params != null)
			post.setEntity(paramsToEntity(params));
		sendRequest(httpClient, httpContext, post, null,
				responseHandler, context);
		//sendRequest(httpClient, httpContext, post, null, responseHandler, context);
	 ...
	 * @param responseHandler 异步响应
	 */
	public  void upload(Context context,String url,String filename, File file, AsyncHttpResponseHandler responseHandler){
		MultipartEntity  entity=new MultipartEntity(HttpMultipartMode.BROWSER_COMPATIBLE);
		entity.addPart(filename,new FileBody(file));
		HttpPost post=new HttpPost(url);
		post.addHeader("charset", HTTP.UTF_8);
	 ...
	 * @param responseHandler 异步响应
	 */
	public  void upload(Context context,String url,String filename, List<File> files, AsyncHttpResponseHandler responseHandler){
		MultipartEntity  entity=new MultipartEntity(HttpMultipartMode.BROWSER_COMPATIBLE);
		for(File file:files){
			entity.addPart(filename,new FileBody(file));
		}
	 ...
	 * @param responseHandler 异步响应
	 */
	public  void upload(String url,String filename, List<File> files, AsyncHttpResponseHandler responseHandler){
		MultipartEntity  entity=new MultipartEntity(HttpMultipartMode.BROWSER_COMPATIBLE);
		for(File file:files){
			entity.addPart(filename,new FileBody(file));
		}
	 ...
	 * @param responseHandler 异步响应
	 */
	public  void upload(String url,String filename, File file, AsyncHttpResponseHandler responseHandler){
		MultipartEntity  entity=new MultipartEntity(HttpMultipartMode.BROWSER_COMPATIBLE);
		entity.addPart(filename,new FileBody(file));
		HttpPost post=new HttpPost(url);
		post.addHeader("charset", HTTP.UTF_8);
	...
	protected void sendRequest(DefaultHttpClient client,
		...
		//把异步请求提交给线程池
		Future<?> request = threadPoolSubmit(new AsyncHttpRequest(client,
		...
		if (context != null) {
			...
			if (requestList == null) {
				...
				requestMapPut(context, requestList);
			}
	...
	protected Future<?>  threadPoolSubmit(AsyncHttpRequest asyncHttpRequest) {
		return threadPool.submit(asyncHttpRequest);
	}
	
	protected  void  requestMapPut(Context context,List<WeakReference<Future<?>>> requestlist) {
		requestMap.put(context, requestlist);
	}
	
	protected  List<WeakReference<Future<?>>>  getrequestMap(Context context) {
		return requestMap.get(context);
	}
