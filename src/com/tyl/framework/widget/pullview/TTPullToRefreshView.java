...
/**
 * 名称：FTPullToRefreshView.java 
 * 描述：下拉刷新和加载更多的View.
 */
public class TTPullToRefreshView extends LinearLayout {
	...
	/** header view. */
	private TTListViewHeader mHeaderView;
	
	/** footer view. */
	private TTListViewFooter mFooterView;
	 ...
	 */
	public TTPullToRefreshView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
	 ...
	 * @param context the context
	 */
	public TTPullToRefreshView(Context context) {
		super(context);
		init(context);
	}
	...
	private void addHeaderView() {
        mHeaderView = new TTListViewHeader(mContext);
	...
	private void addFooterView() {
        ...
        mFooterView = new TTListViewFooter(mContext);
	...
	public boolean onInterceptTouchEvent(MotionEvent e) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			L.d("firstteframework", "onInterceptTouchEvent event.getAction："+deltaY);
			if (isRefreshViewScroll(deltaY)) {
				L.d("firstteframework", "onInterceptTouchEvent event.getAction：事件拦截");
				return true;
			}
			break;
	...
	public boolean onTouchEvent(MotionEvent event) {
		L.d("firstteframework", "event.getAction："+event.getAction());
		...
		case MotionEvent.ACTION_CANCEL:
			...
			L.d("firstteframework", "event.getAction：ACTION_CANCEL"+topMargin+",mHeaderViewHeight:"+mHeaderViewHeight+",mFooterViewHeight:"+mFooterViewHeight);
			if (mPullState == PULL_DOWN_STATE) {
				if (topMargin >= 10-mHeaderViewHeight) {
					// 开始刷新
					headerRefreshing();
				} else {
					// 还没有执行刷新，重新隐藏
				...
				if (Math.abs(topMargin) >= 10 + mFooterViewHeight) {
					// 开始执行footer 刷新
					footerLoading();
				} else {
					// 还没有执行刷新，重新隐藏
	...
	private void headerPrepareToRefresh(int deltaY) {
		...
		if (newTopMargin >= 0 && mHeaderView.getState() != TTListViewHeader.STATE_REFRESHING) {
		    //提示松开刷新
		    mHeaderView.setState(TTListViewHeader.STATE_READY);
		
		} else if (newTopMargin < 0 && newTopMargin > -mHeaderViewHeight) {
		    //提示下拉刷新
		    mHeaderView.setState(TTListViewHeader.STATE_NORMAL);
		}
	}
	...
	private void footerPrepareToRefresh(int deltaY) {
		...
		if (Math.abs(newTopMargin) >= (mHeaderViewHeight + mFooterViewHeight) && mFooterView.getState()  != TTListViewFooter.STATE_LOADING) {
			mFooterView.setState(TTListViewFooter.STATE_READY);
		} else if (Math.abs(newTopMargin) < (mHeaderViewHeight + mFooterViewHeight)) {
		    mFooterView.setState(TTListViewFooter.STATE_LOADING);
		}
	}
	...
	public void headerRefreshing() {
		...
		mHeaderView.setState(TTListViewHeader.STATE_REFRESHING);
	...
	public void onHeaderRefreshFinish() {
		...
		if(mAdapterView!=null){
			...
			if(mCount > 0){
				...
				mFooterView.setState(TTListViewFooter.STATE_EMPTY);
			}
		}else{
			mFooterView.setState(TTListViewFooter.STATE_READY);
		}
	...
	public void onFooterLoadFinish() {
		...
		if(mAdapterView!=null){
			...
			if(countNew > mCount){
				...
				isAll = true;
				mFooterView.setState(TTListViewFooter.STATE_NO);
			}
		}else{
			mFooterView.setState(TTListViewFooter.STATE_READY);
		}
     ...
     */
    public TTListViewHeader getHeaderView() {
        return mHeaderView;
    }
     ...
     */
    public TTListViewFooter getFooterView() {
        return mFooterView;
    }
	...
	public interface OnFooterLoadListener {
		 ...
		 * @param view the view
		 */
		public void onFooterLoad(TTPullToRefreshView view);
	}
	...
	public interface OnHeaderRefreshListener {
		 ...
		 * @param view the view
		 */
		public void onHeaderRefresh(TTPullToRefreshView view);
	}
