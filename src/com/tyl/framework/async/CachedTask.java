package com.tyl.framework.async;
...
public abstract class CachedTask<Params, Progress, Result extends Serializable>
	...
	private Result getResultFromCache() {
		...
		try {
			...
			if (obj != null) {
				L.i(TAG, key+ " read from cache: "+obj);
				return (Result) obj;
			}
		} catch (Exception e) {
			e.printStackTrace();
		...
		L.e(TAG, "read ResultFromCache: fail ");
		return null;
	}
	...
	private boolean saveResultToCache(Result res) {
		...
		L.e(TAG, "save Result To Cache: fail");
		return false;
	}
}
