package com.tyl.framework.util.bitmap;

import java.io.ByteArrayOutputStream;
import java.io.File;

import com.tyl.framework.log.L;
import com.tyl.framework.util.assit.Base64;
import com.tyl.framework.util.dct.FDCT;
import com.tyl.framework.util.math.MathUtility;
...
public class BitMapUtility {
	...
	private final static String TAG="BitMapUtility";
	/**
	 * 描述：获取原图.
	 *
	 * @param file File对象
	 * @return Bitmap 图片
	 */
	public static Bitmap originalImg(String file){ 
		return originalImg(new File(file));
	}
	/**
	 * 描述：获取原图.
	 *
	...
	* @param file File对象
	*/
	public static Bitmap originalImgByCompression(String file){ 
		return originalImgByCompression(new File(file));
	}
	...
	* @param file File对象
	*/
	public static Bitmap originalImgByCompression(File file){ 
		if(!file.exists()){
			return null;
		}
		Bitmap resizeBmp = null;
	...
	/**
	* 描述：图片.压缩
	* @param Bitmap original对象
	*/
	public static Bitmap originalImgByCompression(Bitmap original){ 
		if(original==null){
			return null;
		}
		Bitmap resizeBmp = null;
		...
		// 设置大小
		opts.outHeight = original.getHeight();
		opts.outWidth = original.getWidth();
		//if(D) Log.d(TAG, "将缩放图片:"+file.getPath());
		byte[] imgs=bitmap2Bytes(original,CompressFormat.JPEG,false);
		
		resizeBmp = BitmapFactory.decodeByteArray(imgs, 0, imgs.length, opts);
	    return resizeBmp;
	}
	/**
	* 描述：缩放图片.压缩
	...
	* @return Bitmap 新图片
	*/
	public static Bitmap scaleImg(String file,int newWidth, int newHeight,boolean needRecycle){ 
		return scaleImg(new File(file),newWidth,newHeight,needRecycle);
	}
	...
	* @return Bitmap 新图片
	*/
	public static Bitmap scaleImg(File file,int newWidth, int newHeight,boolean needRecycle){ 
		if(!file.exists()){
			return null;
		}
		Bitmap resizeBmp = null;
	    ...
	    if(resizeBmp != null && scale!=1){
	    	resizeBmp = scaleImg(resizeBmp,scale,needRecycle);
	    }
	 ...
	 * @return Bitmap 新图片
	 */
	  public static Bitmap scaleImg(Bitmap bitmap, int newWidth, int newHeight,boolean needRecycle) {
		   ...
		   if(bitmap == null){
		    ...
		    if(bitmap != null && scale!=1){
		    	resizeBmp = scaleImg(bitmap,scale,needRecycle);
		    }
	        return resizeBmp;
	  }
    ...
    * @return Bitmap 新图片
    */
	  public static Bitmap scaleImg(Bitmap bitmap,float scale,boolean needRecycle){
			Bitmap resizeBmp = null;
			try {
				//获取Bitmap资源的宽和高
				...
				if(needRecycle&&(resizeBmp != bitmap)){
					bitmap.recycle();
				}
			}
	        return resizeBmp;
	...
	*/
    ...
    public static String getHashCode(Bitmap bitmap){
        ...
        //第三步，计算平均值。
        //计算所有64个像素的灰度平均值。
        int avgPixel = MathUtility.average(pixels);
        ...
        for (int i = 0; i < comps.length; i+= 4) {
              ...
              hashCode.append(MathUtility.binaryToHex(result));
        }
    ...
    public static String getDCTHashCode(Bitmap bitmap){
        ...
        //简化色彩。
 		...
 		int[][] pxMatrix =  MathUtility.arrayToMatrix(pixels, width, height);
        double[][] doublePxMatrix = MathUtility.intToDoubleMatrix(pxMatrix);  
        
        //计算DCT,已经变成8*8了
        double[][] dtc = FDCT.fDctTransform(doublePxMatrix);
        
        //计算平均值。
        double[] dctResult =  MathUtility.matrixToArray(dtc);
        int avgPixel = MathUtility.average(dctResult);
    ...
    public static String bitmapToString(Bitmap bitmap) {
        return Base64.encodeToString(bitmap2Bytes(bitmap,Bitmap.CompressFormat.PNG,true), Base64.DEFAULT);
    }
