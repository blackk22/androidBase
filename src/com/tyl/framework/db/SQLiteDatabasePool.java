...
import com.tyl.framework.db.TTSQLiteDatabase.DBParams;
...
public class SQLiteDatabasePool {
	...
	/** 数据库名称*/
	private final static String MATE_DB_NAME="dbName";
	
	/** 数据库版本 */
	private final static String MATE_DB_VERSION="dbVersion";
	...
	public static SQLiteDatabasePool getInstance(Context context) {
		//读取数据库配置信息
		String dbName=MetaDataUtil.readMetaDataString(context, MATE_DB_NAME);
		int dbVersion=MetaDataUtil.readMetaDataInt(context, MATE_DB_VERSION,1);
		L.d("yyz", "dbName:"+dbName+",dbversion:"+dbVersion);
		DBParams params =null;
		if(StringUtils.isEmpty(dbName)){
			params = new DBParams();
		}else{
			params=new DBParams(dbName, dbVersion);
		}
	...
	public synchronized void createPool() {
		...
		L.i(SQLiteDatabasePool.this, " 数据库连接池创建成功！ ");
	}
	...
	private void createSQLiteDatabase(int numSQLiteDatabase) {
		...
		for (int x = 0; x < numSQLiteDatabase; x++) {
			...
			try {
				...
				L.i(SQLiteDatabasePool.this,
						" 创建数据库连接失败！ " + e.getMessage());
			}
			L.i(SQLiteDatabasePool.this, "数据库连接己创建 ......");
		}
	}
	...
	private TTSQLiteDatabase newSQLiteDatabase() {

		// 创建一个数据库连接
		TTSQLiteDatabase sqliteDatabase = new TTSQLiteDatabase(context, params,mDBUpdateListener);
	...
	public synchronized TTSQLiteDatabase getSQLiteDatabase() {
		// 确保连接池己被创建
		if (pSQLiteDatabases == null) {
			return null; // 连接池还没创建，则返回 null
		}

		TTSQLiteDatabase sqliteDatabase = getFreeSQLiteDatabase(); // 获得一个可用的数据库连接
	...
	private TTSQLiteDatabase getFreeSQLiteDatabase() {
		// 从连接池中获得一个可用的数据库连接
		TTSQLiteDatabase sqLiteDatabase = findFreeSQLiteDatabase();
	...
	private TTSQLiteDatabase findFreeSQLiteDatabase() {
		TTSQLiteDatabase sqliteDatabase = null;
	 ...
	 */
	private boolean testSQLiteDatabase(TTSQLiteDatabase sqliteDatabase) {
	...
	public void releaseSQLiteDatabase(TTSQLiteDatabase sqLiteDatabase) {
		// 确保连接池存在，如果连接没有创建（不存在），直接返回
		if (pSQLiteDatabases == null) {
			L.d(SQLiteDatabasePool.this, " 连接池不存在，无法返回此连接到连接池中 !");
			return;
		}
		PooledSQLiteDatabase pSqLiteDatabase = null;
	...
	public synchronized void refreshSQLiteDatabase() {
		...
		// 确保连接池己创新存在
		if (pSQLiteDatabases == null) {
			L.d(SQLiteDatabasePool.this, " 连接池不存在，无法刷新 !");
			return;
		}
	...
	public synchronized void closeSQLiteDatabase() {
		...
		// 确保连接池存在，如果不存在，返回
		if (pSQLiteDatabases == null) {
			L.d(SQLiteDatabasePool.this, "连接池不存在，无法关闭 !");
			return;
		}
		PooledSQLiteDatabase pSqLiteDatabase = null;
		Enumeration<PooledSQLiteDatabase> enumerate = pSQLiteDatabases
	...
	private void closeSQLiteDatabase(TTSQLiteDatabase sqlLiteDatabase) {
	...
	class PooledSQLiteDatabase {
		TTSQLiteDatabase sqliteDatabase = null;// 数据库连接
		boolean busy = false; // 此连接是否正在使用的标志，默认没有正在使用

		// 构造函数，根据一个 TASQLiteDatabase 构告一个 PooledSQLiteDatabase 对象
		public PooledSQLiteDatabase(TTSQLiteDatabase sqliteDatabase) {
			this.sqliteDatabase = sqliteDatabase;
		}

		// 返回此对象中的连接
		public TTSQLiteDatabase getSqliteDatabase() {
			return sqliteDatabase;
		}

		// 设置此对象的，连接
		public void setSqliteDatabase(TTSQLiteDatabase sqliteDatabase) {
			this.sqliteDatabase = sqliteDatabase;
		}
